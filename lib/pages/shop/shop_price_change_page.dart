...
class _ShopPriceChangePageState extends State<ShopPriceChangePage> {
  ...
  TagInfo? _parseTagInfo(dynamic rawTag) {
    if (rawTag is Map) {
      return TagInfo.fromRaw(Map<String, dynamic>.from(rawTag));
    }
    return TagInfo.fromRaw(rawTag);
  }

  List<String> _collectTagLabels(ShopItemAsset item) {
    final schema = _lookupSchema(item);
    ...
    final labels = <String>[];
    for (final key in const ['rarity', 'quality', 'exterior']) {
      final label = _parseTagInfo(tags[key])?.label?.trim();
  ...
  Widget _buildPriceCard(
    ...
    final tagLabels = _collectTagLabels(item);
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (context, index) =>
                  _buildPriceCard(context, currency, _items[index]),
            ),
          ),
        ],
      ),
